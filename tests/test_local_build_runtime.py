...
def test_gui_build_prepares_a_complete_local_cli_runtime() -> None:
    gui_project = (ROOT / "src" / "Satli.Gui" / "Satli.Gui.csproj").read_text(
        encoding="utf-8"
    )
    runtime_script = (
    ...
    assert 'Name="PrepareSatliLocalRuntime"' in gui_project
    ...
    assert 'Join-Path $RuntimeRoot "satli.pyz"' in runtime_script
