...
public sealed class UpdatePackageCleanupServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"satli-update-cleanup-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        var earlier = Path.Combine(root, "SATLI-Setup-v0.6.0.exe");
        var current = Path.Combine(root, "SATLI-Setup-v0.7.1.exe");
        var partial = Path.Combine(root, "SATLI-Setup-v0.7.0.exe.part");
        var future = Path.Combine(root, "SATLI-Setup-v0.8.0.exe");
