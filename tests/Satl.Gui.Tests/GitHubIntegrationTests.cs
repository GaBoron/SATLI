...
public sealed class GitHubIntegrationTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"satl-github-{Guid.NewGuid():N}");
    ...
    {
        var previous = Environment.GetEnvironmentVariable("SATL_GITHUB_CLIENT_ID");
        var directory = Path.Combine(Path.GetTempPath(), $"satl-github-{Guid.NewGuid():N}");
        var path = Path.Combine(directory, "github-auth.json");
        var pollCount = 0;
        try
        {
            Environment.SetEnvironmentVariable("SATL_GITHUB_CLIENT_ID", "Iv1.test-client");
            var service = new GitHubIntegrationService(
        ...
        {
            Environment.SetEnvironmentVariable("SATL_GITHUB_CLIENT_ID", previous);
            if (Directory.Exists(directory))
            {
                Directory.Delete(directory, true);
            }
