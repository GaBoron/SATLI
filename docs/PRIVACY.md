...
- **问题报告、翻译请愿和投稿：** 用户主动操作时，在默认浏览器中打开预填的 GitHub Issue 表单

打开文件报告表单时，预填信息只包含游戏名、App ID、问题类型、用户填写的说明和可选参考来源。请愿和投稿表单只预填已确定的游戏、语言、摘要等字段；ZIP 不会被自动上传，最终内容和附件由用户在浏览器中检查并提交。
...
- **基础：** 记录启动与设置、扫描与缓存、安装与恢复、本地导入、编辑与草稿、修订与导出、Steam 进程保护、网络与更新、GitHub 表单准备等功能的关键开始、结果、用户取消、警告和错误摘要。
- **详细：** 在基础日志上增加 App ID、游戏名、目标语言、项目数量、文件名、逐项处理事件、异常类型和 HRESULT，适合排查具体游戏或操作。
