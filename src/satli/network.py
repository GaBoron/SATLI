...
class NetworkSettings:
    ...
    def from_environment(cls) -> NetworkSettings:
        dns_mode = _choice(os.environ.get("SATLI_DNS_MODE", "system"), {"system", "custom"}, "DNS 模式")
        proxy_mode = _choice(
            os.environ.get("SATLI_PROXY_MODE", "system"),
            {"system", "direct", "manual"},
            "代理模式",
        )
        dns_servers = tuple(_parse_dns_server(item) for item in _split_list(
            os.environ.get("SATLI_DNS_SERVERS", "1.1.1.1; 8.8.8.8")
        ))
        if dns_mode == "custom" and not dns_servers:
            raise NetworkConfigurationError("自定义 DNS 至少需要填写一个服务器地址。")

        proxy_address = os.environ.get("SATLI_PROXY_ADDRESS", "").strip()
        ...
        return cls(
            ...
            dns_timeout=_number("SATLI_DNS_TIMEOUT", 5, 1, 30, "DNS 超时"),
            proxy_mode=proxy_mode,
            proxy_address=proxy_address,
            proxy_username=os.environ.get("SATLI_PROXY_USERNAME", "").strip(),
            proxy_password=os.environ.get("SATLI_PROXY_PASSWORD", ""),
            proxy_bypass=tuple(_split_list(os.environ.get("SATLI_PROXY_BYPASS", ""))),
            proxy_bypass_local=os.environ.get("SATLI_PROXY_BYPASS_LOCAL", "1") != "0",
            connect_timeout=_number("SATLI_CONNECT_TIMEOUT", 15, 3, 120, "连接超时"),
        )
