...
from satli.cache_command import command_cache_refresh
from satli.data_paths import default_data_dir
...
from satli.scan_command import command_scan
from satli.status_command import command_status
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"satli {__version__}")
    ...
    scan.add_argument(
        "--include-owned-games",
        action="store_true",
        help="使用环境变量 SATLI_STEAM_WEB_API_KEY 补全指定账号拥有的游戏",
    )
    scan.add_argument(
        "--scope",
    ...
    status = subparsers.add_parser("status", help="检查 SATLI 管理的安装状态")
...
def _add_data_dir(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--data-dir", type=Path, default=default_data_dir(), help="覆盖 SATLI 数据目录")
