...
from satli.cli_protocol import emit_jsonl, print_catalog_cache_notice
from satli.cli_validation import confirm
from satli.errors import PreflightError, SatliError, TransactionError, UsageError
from satli.models import Catalog, CatalogEntry, SchemaVariant
from satli.steam import discover_local_games, find_steam_dir, is_steam_running, schema_target
...
def command_install(args: argparse.Namespace) -> int:
    ...
    for entry, variant in plan:
        ...
        try:
            ...
            failure = exc if isinstance(exc, SatliError) else TransactionError(
                f"安装 {entry.app_id} 时发生未预期错误：{type(exc).__name__}: {exc}"
            )
            if not isinstance(exc, SatliError):
