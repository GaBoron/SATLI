...
from satli.download_sources import (
...
class CatalogRepository:
    ...
    def _fetch_catalog(self, *, persist: bool = True) -> Catalog:
        ...
        for url in self.catalog_urls:
            try:
                ...
                request_url = f"{url}{separator}satli_refresh={uuid.uuid4().hex}"
    ...
    def download_schema(self, variant: SchemaVariant, *, offline: bool = False) -> Path:
        ...
        failures: list[SatliDownloadFailure] = []
        ...
        for root in self.roots:
            ...
            try:
                ...
                failures.append(SatliDownloadFailure(url, exc, True))
            except (OSError, urllib.error.URLError, TimeoutError) as exc:
                failures.append(SatliDownloadFailure(url, exc, False))
