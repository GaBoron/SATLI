...
public sealed class SchemaEditorService
{
    ...
    public SchemaEditorService(SatliCliService? cli = null)
    {
        ...
        if (!result.IsSuccess)
        {
            throw new InvalidOperationException(
                string.IsNullOrWhiteSpace(result.ErrorMessage)
                    ? $"SATLI schema 操作失败，退出码 {result.ExitCode}。"
                    : result.ErrorMessage);
        }
        return result;
    }
    ...
    private static JsonElement RequiredPayload(CliRunResult result, string eventName) =>
        ...
        ?? throw new InvalidDataException($"SATLI schema 操作未返回 {eventName} 事件。");
