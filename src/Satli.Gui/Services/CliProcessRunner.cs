...
internal sealed class CliProcessRunner
{
    ...
    {
        onDiagnostic?.Invoke("步骤 1：解析 CLI 启动目标。");
        var launch = ResolveLaunch();
        onDiagnostic?.Invoke(
            $"步骤 2：启动目标已解析。可执行文件={launch.FileName}；工作目录={launch.WorkingDirectory}；" +
            $"前置参数={FormatArguments(launch.PrefixArguments)}；附加环境变量={string.Join(",", invocation.Environment.Keys)}");
        ...
        foreach (var environment in invocation.Environment)
        {
            startInfo.Environment[environment.Key] = environment.Value;
        }
        onDiagnostic?.Invoke(
        ...
        onDiagnostic?.Invoke("步骤 4：正在启动 satli 命令行核心。");
        if (!process.Start())
        {
            throw new InvalidOperationException("无法启动 satli 命令行核心。");
        }
        onDiagnostic?.Invoke($"步骤 5：satli 进程已启动。PID={process.Id}。");
    ...
    public static SatliEvent ParseEvent(string line)
    {
        SatliEvent parsed;
        try
        {
    ...
    {
        var overridePath = Environment.GetEnvironmentVariable("SATLI_CLI_PATH");
    ...
    private static string FormatArguments(IEnumerable<string> arguments) =>
        string.Join(
            ...
            arguments.Select(argument => JsonSerializer.Serialize(
                argument,
                SatliJsonSerializerContext.Default.String)));
