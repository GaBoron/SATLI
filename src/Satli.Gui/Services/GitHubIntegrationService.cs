...
public sealed class GitHubIntegrationService
{
    ...
    public string ClientId =>
        Environment.GetEnvironmentVariable("SATLI_GITHUB_CLIENT_ID")?.Trim()
    ...
    private HttpClient CreateClient(string? accessToken = null)
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd(
            $"SATLI/{UpdateService.CurrentVersionText}");
    ...
    private void RequireConfigured()
    {
        if (string.IsNullOrWhiteSpace(ClientId))
        {
            throw new InvalidOperationException(
                "GitHub App Client ID 尚未配置。请注册并安装 GitHub App 后设置 SATLI_GITHUB_CLIENT_ID。");
        }
    }
}
